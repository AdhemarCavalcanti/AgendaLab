const RECURSO_MAP: Record<string, { label: string; cls: string }> = {
  ...
  manutenção: { label: 'Em manutenção', cls: 'bg-(--color-coral-soft) text-(--color-coral) border-(--color-coral)/30' },
}
...
const RESERVA_MAP: Record<string, { label: string; cls: string }> = {
  ...
  aprovada: { label: 'Aprovada', cls: 'bg-(--color-green-soft) text-(--color-green) border-(--color-green)/30' },
